...
const staffId = rest.split(’*’)[0];
const certKey = rest.slice(staffId.length + 1);
const sf = DRIVE_FOLDERS.staff[staffId];
if (!sf) return DRIVE_FOLDERS.root;
if (*REVIEW_CERTS.has(certKey))   return sf.reviews;
if (*CONTRACT_CERTS.has(certKey)) return sf.contract;
return sf.certs;
}
if (key.startsWith(’jd*’))         return DRIVE_FOLDERS.staff[key.replace(’jd*’,’’)]?.contract   || DRIVE_FOLDERS.root;
if (key.startsWith(’extra*’))      return DRIVE_FOLDERS.staff[key.replace(‘extra_’,’’)]?.certs    || DRIVE_FOLDERS.root;
...
return `<tr> <td style="width:42px;vertical-align:top;padding:10px 8px;font-weight:700;font-size:11pt;text-align:center;border:1px solid #d9dde8;">${q.n}</td> <td style="vertical-align:top;padding:10px 12px;font-size:10.5pt;line-height:1.5;border:1px solid #d9dde8;"> <div>${q.q}</div> <div style="font-style:italic;color:#666;font-size:9.5pt;margin-top:4px;">${q.detail}</div> ${detailText ?`<div style="background:#FFF8E6;border-left:3px solid #D4AF37;padding:6px 10px;margin-top:8px;font-size:9.5pt;color:#4a3c1a;">${detailText}</div>` : ""} </td> <td style="width:48px;text-align:center;vertical-align:middle;border:1px solid #d9dde8;">${tick(a==="yes")}</td> <td style="width:48px;text-align:center;vertical-align:middle;border:1px solid #d9dde8;">${tick(a==="no")}</td> <td style="width:48px;text-align:center;vertical-align:middle;border:1px solid #d9dde8;">${tick(a==="na")}</td> </tr>`;
}).join(’’);
...
tbodyRows += `<tr> <td style="padding:7px 10px;font-size:10pt;color:#1a2a24;border:1px solid #c8ddd5;">${c.text}</td> ${cells} <td style="padding:5px 6px;text-align:center;font-weight:700;font-size:10pt;color:${pctColor};background:${pctBg};border-left:2px solid #c8ddd5;">${pctText}</td> </tr>`;
});
...
return `<tr style="background:${bg};"><td style="padding:6px 14px;">${label}</td> <td style="text-align:center;width:72px;padding:6px;">${cell}</td> <td style="width:200px;font-size:9.5pt;color:#666;padding:6px 12px;">${noteCell}</td></tr>`;
}).join(’’);
...
<tr><th>Auditor</th><td>${audit.signature
  ? `<img src="${sigSrcFor(audit, audit.signature)}" alt="signature" style="max-height:42px;max-width:260px;display:inline-block;vertical-align:middle;"/>&nbsp;&nbsp;&nbsp;<span style="font-size:9.5pt;color:#666">${audit.signedBy||audit.auditor} · ${fmtNZ(audit.date)}</span>`
...
: _tick(true,‘2025b’);
const noteCell = note ? note : (isFail ? ‘See notes’ : ‘—’);
return `<tr style="background:${bg};"><td style="padding:7px 14px;">${label}</td> <td style="text-align:center;width:72px;padding:7px;">${cell}</td> <td style="width:200px;font-size:9.5pt;color:#666;padding:7px 12px;">${noteCell}</td></tr>`;
}).join(’’);
...
<tr><th>Auditor</th><td>
  ${audit.signature
    ? `<div class="attestation" style="padding:10px 16px;">
         <div style="margin-bottom:4px;"><img src="${sigSrcFor(audit, audit.signature)}" alt="signature" style="max-height:56px;max-width:260px;display:block;"/></div>
...
const driveFile = await *uploadFileToDrive(’mtgatt*’+m.id, fileName, ‘text/html’, dataUrl);
if (driveFile) {
...
if (cert?.auditBased) {
const name=STAFF[id]?.name||””;
const auditList=*portalStore.data[“audits”]||[];const found=auditList.some(a=>a.type===“clinical_notes”&&a.physioAudited===name);
...
if (f) return “ok”;
const name = STAFF[id]?.name || “”;
const auditList = *portalStore.data[“audits”] || [];
...
const f = loadFile(id, key);
// For JD: also check the Documents tab storage (jd*${id})
if (!f && key === “jd”) {
const docJd = loadGen(“jd*” + id);
if (docJd && (Array.isArray(docJd) ? docJd.length > 0 : true)) return “ok”;
}
if (!f) return “pending”;
const expiryData = _portalReady ? (*portalStore.data[“expiry*”+sKey(id,key)] || null) : null;
...
if(_portalReady){ delete *portalStore.data[“expiry*”+key]; _debouncedSave(); }
...
if(expiry||issued){ *portalStore.data[“expiry*”+key]={expiry,issued}; }
_debouncedSave();
_scheduleForceUpdate();
}
setScanning(false);
...
const expiryData=_portalReady?(*portalStore.data[“expiry*”+sKey(staffId,cert.key)]||null):null;
...
if(*portalStore.data[“expiry*”+key]){delete *portalStore.data[“expiry*”+key];_debouncedSave();}
...
if(_portalReady){
*uploadFileToDrive(gkey+”*”+id,f.name,f.type,ev.target.result).then(driveFile=>{
if(driveFile){
setFiles(prev=>{
...
const merged=[…(certFile?[{…certFile,*src:“profile”}]:[]),…extraFiles.map(f=>({…f,*src:“docs”}))];
function handle(e){
const f=e.target.files[0];if(!f)return;
if(f.size>3*1024*1024){alert(“File over 3MB.”);return;}
const r=new FileReader();
...
const updated=[…extraFiles,d];setExtraFiles(updated);saveGen(“jd*”+staffId,updated);
if(*portalReady){
*uploadFileToDrive(“jd*”+staffId+”*”+d.id,d.fileName,d.fileType,d.dataUrl).then(driveFile=>{
if(driveFile){
setExtraFiles(prev=>{const up=prev.map(x=>x.id===d.id?{…x,…driveFile,dataUrl:undefined}:x);saveGen(“jd*”+staffId,up);return up;});
}
}).catch(()=>{});
}
};
...
if(_portalReady){
*uploadFileToDrive(key+”*”+d.id,d.fileName,d.fileType,d.dataUrl).then(driveFile=>{
if(driveFile){
...
const blobFile=await *uploadToBlob(“logauditevid*”+Date.now(),f.name,f.type,ev.target.result);
...
if(_portalReady){
const driveFile=await *uploadFileToDrive(“auditevid*”+audit.id,f.name,f.type,ev.target.result);
...
if(_portalReady){
const driveFile=await *uploadFileToDrive(“auditevid*”+target.id,f.name,f.type,dataUrl);
if(driveFile)Object.assign(evidence,driveFile);
...
const r=new FileReader();
r.onload=async ev=>{
const blobFile=await *uploadToBlob(“mtgatt*”+meeting.id,f.name,f.type,ev.target.result);
    ...
    <div style={{padding:"22px 28px 24px",maxHeight:"72vh",overflowY:"auto"}}>
      ...
      <div style={{marginTop:28,paddingTop:20,borderTop:`1px solid ${C.grayL}`,display:"flex",justifyContent:"space-between",alignItems:"flex-end",gap:24}}>
        <div>
          ...
          {audit.signature
            ? <>
                <img src={sigSrcFor(audit, audit.signature)} alt="signature" style={{maxHeight:50,maxWidth:220,display:"block"}}/>
        ...
        <div>
          ...
          {audit.signature
            ? <div style={{borderBottom:"1px solid #333",paddingBottom:3,minHeight:36,display:"flex",alignItems:"flex-end"}}>
                <img src={sigSrcFor(audit, audit.signature)} alt="signature" style={{maxHeight:36,maxWidth:220,display:"block"}}/>
              </div>
      ...
      <div style={{marginTop:26}}>
        <div style={{fontSize:12}}>
          Signed: {audit.signature
            ? <img src={sigSrcFor(audit, audit.signature)} alt="signature" style={{maxHeight:30,maxWidth:180,verticalAlign:"middle",marginLeft:4}}/>
...
const driveFile=await *uploadFileToDrive(“ppdoc*”+id,f.name,f.type,ev.target.result);
if(driveFile){
